...
class Net(nn.Module):
    ...
    def __init__(self, genome, D=3, K=4):
        ...
        for gene in genome:
            if gene.get('type') == 'conv1d':  # Add conv1d layer
                layers.append(nn.Conv1d(input_features, gene['num_neurons'], kernel_size=3, stride=1, padding=1))
            ...
            if gene.get('batch_norm', False):  # Optional batch normalization
                layers.append(nn.BatchNorm1d(input_features))

            if gene.get('dropout_rate') is not None:
                layers.append(nn.Dropout(gene['dropout_rate']))

        layers.append(nn.Linear(input_features, K))  # Final output layer
...
def generate_initial_population(size, blueprint):
    ...
    for _ in range(size):
        genome = []
        for _ in range(random.randint(1, blueprint['max_n_layers'])):  # Random number of layers
            layer = {
...
def crossover(parent1, parent2):
    ...
    for layer1, layer2 in zip(parent1, parent2):
        layer = {
            'type': random.choice([layer1.get('type', 'linear'), layer2.get('type', 'linear')]),
            'num_neurons': random.choice([layer1['num_neurons'], layer2['num_neurons']]),
            'activation': random.choice([layer1['activation'], layer2['activation']]),
            'dropout_rate': random.choice([layer1['dropout_rate'], layer2['dropout_rate']])
        }
        child.append(layer)
    return child
...
def mutate(genome,mutation_rate = 0.1):
    ...
    for layer in genome:
        if random.random() < mutation_rate:
            layer['type'] = random.choice(blueprint['layer_types'])
        if random.random() < mutation_rate:
...
blueprint = {
    ...
    'neurons': [32, 64, 128, 256,512],              # Possible neuron counts per layer
    'activations': ['sigmoid','relu','leaky_relu'],  # Possible activation functions
    'dropout': [None, 0.1, 0.25, 0.5],            # Possible dropout rates, including 'None' for no dropout
    'layer_types': ['linear'] 
}
